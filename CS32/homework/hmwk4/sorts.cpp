...
bool compareStorePtr(const Store* lhs, const Store* rhs)
{
    ...
    // what aaron did: one line and just took the parameters and dereferenced them
        // hope that works 
...
void insertion_sort(vector<Store>& s, bool comp(const Store&, const Store&))
{
    ...
    for (int unsorted = 0; unsorted < s.size(); unsorted++)
    {
        ...
        int loc = unsorted;
        while (loc > 0 && compareStore(s[loc-1], nextItem))
        {
            s[loc] = s[loc - 1];
            loc--;
        }
...
void sortUsingPtrs(vector<Store>& stores, bool comp(const Store*, const Store*))
{
    ...
    // creating a vector of the copy of the stores vector passed in the parameter called auxStores

      // TODO:  Create a vector of Store pointers, and set each pointer
      //        to point to the corresponding Store in auxStores.
    vector<Store*> VecStorePtrs;
    int i = 0;
    while (i < auxStores.size())
    {
        VecStorePtrs.push_back(&auxStores[i]);
        i++;
    }

      // TODO:  Sort the vector of pointers using the STL sort algorithm
      //        with the comp parameter as the ordering relationship.
    sort(VecStorePtrs.begin(), VecStorePtrs.end(), comp);
    // insertion_sort(VecStorePtrs, comp);
    ...
    for (int i = 0; i < VecStorePtrs.size(); ++i) 
    {
        stores[i] = *VecStorePtrs[i];
    }
